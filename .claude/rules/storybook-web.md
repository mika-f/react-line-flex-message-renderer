...
6. **Use `pnpm` for installs and `pnpm exec storybook` for Storybook/Vitest CLI commands** (detected from this project's lockfile). Do not use `npx` — it invokes npm and fails when the repo enforces a different package manager.
...
3. Re-run vitest **only for the affected file(s)**: `pnpm exec vitest --project storybook run path/to/Foo.stories.tsx`.
...
- `pnpm exec vitest --project storybook run` passes for the new files.
